...
export function taxInvestorChannels(ctx: PolicyContext): PolicyChannelDelta {
  ...
  const ss = clamp(t.shortStayRegulationIntensity, 0, 1) * ramp;
  if (ss > 0) {
  ...
  const fb = clamp(t.foreignBuyerRestrictionIntensity, 0, 1) * ramp;
  ...
  const shift = clamp(t.landTaxShift, 0, 1) * ramp;
  if (shift > 0) {
