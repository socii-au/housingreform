...
export function planningChannels(ctx: PolicyContext): PolicyChannelDelta {
  ...
  const up = clamp(p.upzoningIntensity, 0, 1) * ramp;
  if (up > 0) {
    // Upzoning unlocks feasible supply but rolls out slowly.
    d.completionsMultiplier *= 1 + 0.10 * up; // up to +10% baseline completions
  }

  const infra = clamp(p.infrastructureEnablement, 0, 1) * ramp;
  const lag = Math.max(0, Math.round(p.infrastructureLagYears ?? 3));
  if (infra > 0) {
    // Infrastructure lifts capacity after a lag, ramping in over 2 years after lag.
    const rampAfterLag = Math.max(0, ctx.yearIndex - lag);
    const active = Math.min(1, rampAfterLag / 2);
